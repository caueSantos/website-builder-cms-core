...
$.widget("custom.dataOwlCarousel", {
  ...
  _parseOptions: function () {
    ...
    if(!finalOptions['items']){
      var items = 1;
      if(finalOptions['rwd']){
        items = finalOptions['rwd'].split('-').reverse()[0];
      }
      finalOptions['items'] = items;
    }

    if (finalOptions['forceRun'] != true && shouldRun(finalOptions['items']) === false) {
      finalOptions['autoplay'] = false;
      finalOptions['loop'] = false;
      // finalOptions['mouseDrag'] = false;
      finalOptions['nav'] = false;
      finalOptions['dots'] = false;
      finalOptions['navContainer'] = false;

    }

    if ($(_this.element).find('.item').length < finalOptions['items']) {

      $(_this.element).addClass('owl-incomplete');
      finalOptions['center'] = true;

      $(_this.element).siblings('.carousel-super-nav').hide(0);

    }

    if (data.owlNavText != null) {
      finalOptions['navText'] = data.owlNavText.split('--');
    }

    if (data.owlNavText != null) {
      var navs = data.owlNavText.split('--');
      $.each(navs, function (i, v) {
        if (v.indexOf('@icon:') > -1) {
          v = v.replace('@icon:', '<i class="').replace('@', '"></i>');
          navs[i] = v;
        }
      });
      finalOptions['navText'] = navs;
    }

    var rwdOptions = {};
    if (finalOptions['rwd'] != null) {
      var itemsArr = finalOptions['rwd'].split('-');
      rwdOptions = {
        responsive: {
          0: {
            items: itemsArr[0],
    ...
    //console.log($.extend(true, {}, rwdOptions, this.options, finalOptions));
...
$('.owl-carousel[data-owl-carousel]').dataOwlCarousel();
