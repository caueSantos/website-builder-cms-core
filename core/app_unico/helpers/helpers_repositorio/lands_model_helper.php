...
if (!function_exists('lands_debug')) {



    function lands_debug($id_sessao, $dados = null) {
        ...
        $sessao[0]->ultima_atividade = now();
        if ($dados == null) {
            $sessao[0]->dbug.= "[" . hora() . "] Executou ( ";
            if ($CI->uri->segment(1)) {

                $sessao[0]->dbug.= "  controller -> " . $CI->uri->segment(1);
            }

            if ($CI->uri->segment(2))
                $sessao[0]->dbug.= " / funcao -> " . $CI->uri->segment(2);


            if ($CI->uri->segment(3))
                $sessao[0]->dbug.= " /param(3) -> " . $CI->uri->segment(3);

            if ($CI->uri->segment(4))
                $sessao[0]->dbug.=" /param(4) -> " . $CI->uri->segment(4);

            if ($CI->uri->segment(5))
                $sessao[0]->dbug.=" /param(5) ->" . $CI->uri->segment(5);

            if ($CI->uri->segment(1))
                $sessao[0]->dbug.="  ) <br>"; else
                $sessao[0]->dbug.=" nada ) <br>";
        }

        $updt = object_to_array($sessao[0]);

        $CI->model_sessao->atualiza($sessao[0]->id, $updt);



        return $CI->model_sessao->get_debug($sessao[0]->id);
    }
