...
class Model_contato_novo extends CI_Model {
...
//                ver($_FILES['Arquivos_arq']);
                ...
                for ($i = 0; $i < $items; $i++) {
                    ...
                    if ($name) {
                        $ext = strtolower(pathinfo($name, PATHINFO_EXTENSION));
                        $new_name = $this->tabela_emails_contato . '_' . $name . '_' . date("Y.m.d-H.i.s") . '.' . $ext;
        ...
        $this->smarty->assign('contato',$contato_inserido[0]);
