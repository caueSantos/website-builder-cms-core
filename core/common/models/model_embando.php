...
class Model_embando extends Model_banco_cliente {
...
//  public $tabela_emails_proprietario = 'contato';
    ...
    function inicializa($promocao) {
        ...
        $this->namespace_promocao = $promocao[0]->Namespace_promocao_url;
    }
            ...
            foreach ($bloqueados as $bloqueado) {
                ...
                if ($bloqueado->Facebook_id_txf == $user) {
                    ...
                    if ($bloqueado->Namespace_empresa_sel == $this->namespace_empresa) {
                        if ($bloqueado->Namespace_promocao_url == $this->namespace_promocao || $bloqueado->Namespace_promocao_url == 'TODAS' || $bloqueado->Namespace_promocao_url == '') {
                            $bloqueio = true;
                        }
                    }
    ...
    function verifica_inscricao_dupla($user = null) {
        ...
        $sql = "select * from {$this->tabela_inscricoes} where Facebook_id_txf='{$user}' and Namespace_empresa_txf='{$this->namespace_empresa}' and Namespace_promocao_url='{$this->namespace_promocao}' ";
    ...
    function verifica_promocao_encerrada($promocao = null) {
        ...
        if ($promocao[0]->Fim_dat == date('Y-m-d')) {
            return $this->verifica_hora($promocao);
        }
    }

    function verifica_hora($promocao) {
        $hora = date('H:i');

        $hora_promocao = $promocao[0]->Hora_txf;
        if ($hora_promocao >= $hora) {
            return false;
        } else {
            return true;
        }
    ...
    function verifica_sexo_invalido($usuario = null, $promocao = null) {
        ...
        if ($promocao[0]->Sexo_permitido_sel == 'TODOS') {
            ...
            if ($sexo == $promocao[0]->Sexo_permitido_sel) {
