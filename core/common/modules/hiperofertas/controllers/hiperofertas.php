...
class hiperofertas extends lands_face {
...
//seta usuario
    ...
    function configura_usuario() {
         //ver('configura_usuario',1);
        
        
        $id = $this->user_profile['id'];
//        //ver($this->user_profile);
...
//        $registro['Data_hora_dat'] = $dia . ' ' . $hora;
              ...
              $array['Data_dat'] = date('Y-m-d');
        $array['Data_hora_dat'] = date('Y-m-d h:i:s');
...
////ver('chgeo');
        switch ($this->pagina_atual) {
            ...
            case 'minhas_ofertas':
                ...
                foreach ($objeto->registros as $inscricao) {
                    $inscricao->Promocao = $this->mbc->buscar_completo("promocoes", "where Namespace_empresa_sel='{$inscricao->Namespace_empresa_txf}' and Namespace_promocao_txf='{$inscricao->Namespace_promocao_txf}'");
    ...
    function carrega_namespace_empresa() {
        //ver('carrega namespace empresa',1);
    ...
    function carrega_empresa($namespace = 'flex') {
          //ver('carrega empresa',1);
        if (isset($namespace)) {
...
//                                    group by Facebook_id_txf order by Total_txf desc , Data_hora_dat  ";
            ...
            case 'sexo_invalido':
                $segment2 = $this->uri->segment($this->app->Segmento_padrao_txf + 2);
                if (!$segment2) {
                    die('Sexo invalido.... Faltando Id da promocao.');
                }
                $promocao = $this->mbc->buscar_completo("promocoes", "where Id_int=$segment2");
                $this->promocao = $promocao[0];
                $this->smarty->assign('promocao', $promocao);
...
//                        $this->model_smarty->carrega_bloco('totalizadores_acessos', 'totalizadores_acessos', $this->app->Template_txf);
            ...
            case 'inscricoes':
                ...
                if (!isset($status[0]->Id_int)) {
                    $status[0]->Nome_txf = '24 HORAS';
                    $status[1]->Nome_txf = 'AGUARDANDO';
                    $status[2]->Nome_txf = 'CANCELADO';
                    $status[3]->Nome_txf = 'CONFIRMADO';
                    $status[4]->Nome_txf = 'FIDELIZADO';
                }
...
////ver($totalizadores);
    ...
    function abre_promocao() {
        ...
        //ver('abre promocao',1);
$query="where Namespace_empresa_sel='{$this->namespace_empresa}' and Namespace_promocao_txf='{$this->namespace_promocao}'";
        $promocao = $this->mbc->buscar_completo("promocoes", "$query");
    ...
    function verifica_seguranca_promocao($promocao) {
        //ver('verifica_seguranca_promocao');
    ...
    function carrega_bloco_formulario() {
        //ver('carrega_bloco_formulario',1);
