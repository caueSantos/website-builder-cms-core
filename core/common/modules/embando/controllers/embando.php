...
require_once(COMMONPATH . 'core/lands_embando.php');

class embando extends lands_embando {
    ...
    function ajax() {
        ...
        switch ($this->pagina_atual) {
            case 'datas':
                ...
                $datas_promocao = $this->busca_datas_promocao($_POST['Id_promocao']);
                foreach ($datas_promocao as $data) {
                    $data->Ocupacao = array();
                    $data->Ocupacao = $this->mbc->executa_sql("select * from datas_ocupacao where Id_data={$data->Id_int} and Id_inscricao={$_POST['Id_inscricao']}");
                }

                $this->smarty->assign('Id_inscricao', $_POST['Id_inscricao']);
                $this->smarty->assign('Id_promocao', $_POST['Id_promocao']);
                $this->smarty->assign('datas_promocao', $datas_promocao);

                $this->model_smarty->render_ajax('datas', $this->app->Template_txf);
                die();
                break;
            default: echo "chegou no default";
                break;
        }
    }
        ...
        switch ($this->pagina_atual) {
            case 'promocoes':
                ...
                foreach($objeto->registros as $registro){
                    $registro->Datas=$this->busca_datas_promocao($registro->Id_int);
                }
            ...
            case 'minhas_ofertas':
                ...
                foreach ($objeto->registros as $inscricao) {
                    $inscricao->Promocao = $this->mbc->buscar_completo("promocoes", "where Namespace_empresa_sel='{$inscricao->Namespace_empresa_txf}' and Namespace_promocao_url='{$inscricao->Namespace_promocao_url}'", 'Miniatura_ico');
 ...
 foreach($objeto->registros as $registro){
            ...
            case 'admin':
                ...
                foreach ($objeto->registros as $promocao) {
                    $sql = "select COUNT(*) as total, t.* from inscricoes t where t.Namespace_empresa_txf='{$promocao->Namespace_empresa_sel}' and t.Namespace_promocao_url='{$promocao->Namespace_promocao_url}' ";
                    $inscricoes = $this->mbc->executa_sql($sql);
                    $promocao->Total_inscritos_txf = $inscricoes[0]->total;
                    $sql2 = "select COUNT(*) as total, t.* from acessos t where t.Namespace_empresa_txf='{$promocao->Namespace_empresa_sel}' and t.Namespace_promocao_url='{$promocao->Namespace_promocao_url}' ";
            ...
            case 'inscricoes_sorteio':
                ...
                $inscricoes = $this->mbc->executa_sql("select * from inscricoes  where Namespace_empresa_txf='{$this->promocao->Namespace_empresa_sel}' and Namespace_promocao_url='{$this->promocao->Namespace_promocao_url}' order by Status_sel desc, Nome_txf");
                $this->smarty->assign('inscricoes', $inscricoes);
                $sorteios = $this->mbc->executa_sql("select s.*, s.Id_int as Id_sorteio,  i.* from sorteios s  left outer join inscricoes i on i.Id_int=s.Inscricao_int where s.Namespace_empresa_txf='{$this->promocao->Namespace_empresa_sel}' and s.Namespace_promocao_url='{$this->promocao->Namespace_promocao_url}' order by s.Id_int , Nome_txf");
...
//                                and t.Namespace_promocao_url='{$this->promocao->Namespace_promocao_url}' 
//                                    group by Facebook_id_txf order by Total_txf desc , Data_hora_dat  ";
                ...
                $acessos_total = $this->mbc->executa_sql("SELECT *   FROM acessos where Namespace_empresa_txf='{$this->promocao->Namespace_empresa_sel}' and Namespace_promocao_url='{$this->promocao->Namespace_promocao_url}'");
...
//                }
            ...
            case 'imprime_inscricoes':
                ...
                $totalizadores = $this->mbc->executa_sql("select  COUNT(t.Status_sel) as total, t.Status_sel from inscricoes t  where t.Namespace_empresa_txf='{$this->promocao->Namespace_empresa_sel}' and t.Namespace_promocao_url='{$this->promocao->Namespace_promocao_url}' group by Status_sel order by t.Status_sel desc, t.Nome_txf");
...
//ver($totalizadores);
            ...
            case 'imprime_acessos':
                ...
                $this->promocao = $promocao[0];
                $this->smarty->assign('promocao', $promocao);
//                $sql2 = "select COUNT(*) as Total_txf , t.* from acessos t where t.Namespace_empresa_txf='{$this->promocao->Namespace_empresa_sel}' and t.Namespace_promocao_url='{$this->promocao->Namespace_promocao_url}' group by Facebook_id_txf order by Total_txf desc , Data_hora_dat  ";
...
//                        $this->model_smarty->carrega_bloco('totalizadores_acessos', 'totalizadores_acessos', $this->app->Template_txf);
            ...
            case 'exportar_acessos':
                ...
                $sql2 = "select COUNT(*) as Total , t.Namespace_promocao_url as Promocao, t.Nome_txf as Nome, CONCAT('http://facebook.com/',t.Facebook_id_txf) as Facebook from acessos t where t.Namespace_empresa_txf='{$this->promocao->Namespace_empresa_sel}' and t.Namespace_promocao_url='{$this->promocao->Namespace_promocao_url}' group by Facebook_id_txf order by Total desc, Data_hora_dat  ";
                $acessos = $this->mbc->executa_sql($sql2);
            ...
            case 'imprime_ticket':
                ...
                $this->smarty->assign('promocao', $promocao);
                $this->promocao = $promocao[0];
                $inscricao = $this->mbc->executa_sql("select * from inscricoes where Facebook_id_txf='" . $this->fbuser . "'  and Namespace_empresa_txf='{$promocao[0]->Namespace_empresa_sel}' and Namespace_promocao_url='{$promocao[0]->Namespace_promocao_url}' limit 1");
            ...
            case 'imprime_ticket_admin':
                ...
                $this->smarty->assign('inscricao', $inscricao[0]);
                $promocao = $this->mbc->buscar_completo("promocoes", "where Namespace_empresa_sel='{$inscricao[0]->Namespace_empresa_txf}' and Namespace_promocao_url='{$inscricao[0]->Namespace_promocao_url}'");
                $this->smarty->assign('promocao', $promocao);
                $this->promocao = $promocao[0];
                $this->model_smarty->carrega_bloco('impressao_topo', 'impressao_topo', $this->app->Template_txf);
                $this->model_smarty->carrega_bloco('regulamento', 'regulamento', $this->app->Template_txf);
            ...
            case 'inscricoes':
                ...
                $inscricoes = $this->mbc->executa_sql("select * from inscricoes  where Namespace_empresa_txf='{$this->promocao->Namespace_empresa_sel}' and Namespace_promocao_url='{$this->promocao->Namespace_promocao_url}' {$wherepost} order by Status_sel desc, Nome_txf");

                foreach ($inscricoes as $inscricao) {
                    $inscricao->Datas = array();
                    $inscricao->Datas = $this->busca_datas_inscricao($inscricao->Id_int);
                }
...
//ver($totalizadores);
    ...
    function abre_promocao() {
        ...
        $promocao = $this->mbc->buscar_completo("promocoes", "where Namespace_empresa_sel='{$this->namespace_empresa}' and Namespace_promocao_url='{$this->namespace_promocao}'");
        ...
        if (!isset($promocao[0]->Id_int)) {
            ...
            $promocao[0]->Datas=$this->busca_datas_promocao($id_promocao,' Data_dat>=CURRENT_DATE() and ');
    ...
    function busca_configuracoes_pagto($admin = null) {
        $promocao_configs = $this->mbc->buscar_completo("promocoes_configs", "where Namespace_empresa_sel='{$this->promocao->Namespace_empresa_sel}' and Namespace_promocao_url='{$this->promocao->Namespace_promocao_url}'");
    ...
    function busca_inscricao($promocao) {
        ...
        $this->busca_datas_inscricao($this->inscricao->Id_int);
    ...
    function verifica_seguranca_promocao($promocao) {
        $this->carrega_model('model_embando');
        $this->model_embando->inicializa($promocao);

        if ($this->model_embando->verifica_inscricao_dupla($this->fbuser, $promocao[0]->Id_int)) {
            redirect($this->app->Url_cliente . $this->namespace_empresa . "/duplicado/" . $promocao[0]->Id_int);
        }

        if ($this->model_embando->verifica_usuario_bloqueado($this->fbuser)) {
            redirect($this->app->Url_cliente . $this->namespace_empresa . "/bloqueado/" . $promocao[0]->Id_int);
        }

        if ($this->model_embando->verifica_promocao_encerrada($promocao)) {
            redirect($this->app->Url_cliente . $this->namespace_empresa . "/encerrada/" . $promocao[0]->Id_int);
        }

        if ($this->model_embando->verifica_promocao_nao_iniciada($promocao)) {
            redirect($this->app->Url_cliente . $this->namespace_empresa . "/aguarde/" . $promocao[0]->Id_int);
        }
//        if ($this->model_embando->verifica_sexo_invalido($this->user_profile, $promocao)) {
...
//        }
    ...
    function carrega_bloco_formulario() {
        ...
        if ($this->promocao[0]->Bloco_formulario_sel == 'padrao') {
            $this->model_smarty->carrega_tpl_modulo($this->app->Template_txf, 'formularios', 'padrao', 'formulario');
        } else {
            $this->model_smarty->carrega_tpl_modulo($this->app->Template_txf, 'formularios/' . $this->namespace_empresa, $this->promocao[0]->Bloco_formulario_sel, 'formulario');
        }
    }
    ...
    function enviar() {
        ...
        switch ($this->uri->segment($segmento)) {
            case 'inscricao':
                ...
                $this->namespace_promocao = $_POST['Namespace_promocao_url'];
                $promocao = $this->mbc->buscar_completo("promocoes", "where Namespace_empresa_sel='{$this->namespace_empresa}' and Namespace_promocao_url='{$this->namespace_promocao}'");
...
//                        }
                $inseriu = $this->model_embando->insere_inscricao();
                ...
                if ($inseriu['resultado'] == 'ok') {
                        ...
                        if ($_POST['Datas'][0]) {
                            $sql = "select * from inscricoes where Email_txf='{$_POST['Email_txf']}' and Namespace_empresa_txf='{$_POST['Namespace_empresa_txf']}' and Namespace_promocao_url='{$_POST['Namespace_promocao_url']}'";

                            $id_inscricao = $this->mbc->executa_sql($sql);


                            foreach ($_POST['Datas'] as $data) {
                                $post_array['Id_data'] = $data;
                                $post_array['Id_inscricao'] = $id_inscricao[0]->Id_int;
                                $post_array['Id_promocao'] = $_POST['Id_promocao'];
                                $post_array['Status_sel'] = 'PENDENTE';

                                $this->mbc->db_insert('datas_ocupacao', $post_array);
                            }
                            $this->busca_datas_inscricao($id_inscricao[0]->Id_int);
                        }
            ...
            case 'troca_status':
                ...
                $this->carrega_model('model_embando');
                $this->model_embando->inicializa($promocao);
                $this->model_embando->atualiza_registro();
                redirect($this->app->Url_cliente . $this->namespace_empresa . '/inscricoes/' . $id_promocao);
                break;
            case 'alterar_data':
               
                if ($_POST['Id_int']) {
                    if ($this->mbc->updateTable('datas_ocupacao', $_POST, 'Id_int', $_POST['Id_int'])) {
                    ...
                    if ($_POST['Status_sel'] != 'NAO') {
                        if ($this->mbc->db_insert('datas_ocupacao', $_POST)) {
                            $this->smarty->assign('mensagem', 'atualizacao_ok');
                            $this->model_smarty->render_ajax('mensagens_cadastro', $this->app->Template_txf);
                            die();
                        } else {
            ...
            case 'cancelar_inscricao':
                ...
                if ($this->mbc->db_delete('inscricoes', 'Id_int', $id_inscricao)) {
                    $this->mbc->db_delete('datas_ocupacao', 'Id_inscricao', $id_inscricao);
            ...
            case 'sorteio':
                ...
                $sql = "select *, Id_int as Inscricao_int from inscricoes 
                              ...
                              and Namespace_promocao_url='{$this->promocao->Namespace_promocao_url}' 
                              and Id_int NOT IN 
                              ( select Inscricao_int from sorteios 
                              where Namespace_empresa_txf='{$this->promocao->Namespace_empresa_sel}'  
                              and Namespace_promocao_url='{$this->promocao->Namespace_promocao_url}' ) order by RAND() limit 1";
...
//                        redirect($this->app->Url_cliente . $this->namespace_empresa . '/inscricoes/' . $id_promocao);
    ...
    function busca_datas_promocao($id_promocao,$where_data=null) {
        $datas_promocao = $this->mbc->executa_sql("select * from datas  where $where_data Tabela_con='promocoes' and Id_objeto_con={$id_promocao} order by Data_dat, Id_int");

        $this->smarty->assign("datas_promocao", $datas_promocao);
        return $datas_promocao;
    }

    function busca_datas_inscricao($id_inscricao) {
        $datas_inscricao = $this->mbc->executa_sql("select d.* from datas d left outer join datas_ocupacao do on do.Id_data=d.Id_int where do.Id_inscricao={$id_inscricao}");
        $this->smarty->assign("datas_inscricao", $datas_inscricao);
        return $datas_inscricao;
    }

    function altera_registro() {
        $this->carrega_model('model_embando');
//            $this->model_embando->inicializa($promocao);
        if ($this->model_embando->atualiza_registro()) {
            $this->smarty->assign('mensagem', 'atualizacao_ok');
    ...
    function grava_log_acesso($pagina = null) {
        ...
        $array['Namespace_promocao_url'] = $this->namespace_promocao;
