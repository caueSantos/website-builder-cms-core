...
//        $this->model_moip->inicia_moip(3);
        ...
        switch ($this->pagina_atual) {
            ...
            case 'recebimentos_bbq':

                $recebimentos = $this->mbc->executa_sql("select * from recebimentos where produto='Ingresso(s) FESTIVAL DE CHURRASCO - SERRANO BBQ' and status='pago'  order by Id_int desc");
                foreach($recebimentos as $recebimento){
                    $retornos=$this->mbc->executa_sql("select * from log_retornos where id_recebimento = '$this->recebimento->Id_int'");
                }                
                $this->smarty->assign('recebimentos', $recebimentos);
                break;
        }
