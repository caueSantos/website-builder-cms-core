...
require_once(COMMONPATH . 'core/lands_pitacos.php');

class pitacos extends lands_pitacos {

    public function __construct() {
        parent::__construct();
//        $this->load->helper('tradutor');
        $this->load->model('model_pitacos');
        $this->model_pitacos->inicializa($this->app, $this->cliente);
...
//            $usuario_novo = $this->mbc->executa_sql("select * from users where Id_int={$usuario->Id_int}");
        ...
        $regulamento = $this->mbc->buscar_completo('regulamento', "where Id_int is not null");
...
//            $this->model_smarty->carrega_bloco('anuncios_baixo', 'anuncios_baixo', $this->app->Template_txf);
    ...
    function switch_pagina() {
        ...
        if ($this->usuario->Id_int) {
            $notificacoes = $this->model_pitacos->busca_notificacoes();
        ...
        switch ($this->pagina_atual) {
            ...
            case 'inicio' :
                ...
                $outras_rodadas = $this->model_pitacos->busca_rodadas_encerradas();
                //ver('chegou');
                if ($outras_rodadas) {
...
//                ver($ultimo_ranking);
            ...
            case 'meus_desafios':
                ...
                $todas_rodadas = $this->model_pitacos->busca_rodadas();
                $meus_desafios = $this->model_pitacos->busca_desafios_usuario($this->usuario->Id_int);
                ...
                $meus_desafios_pendentes = $this->model_pitacos->busca_solicitacoes_desafios_usuario($this->usuario->Id_int);
            ...
            case 'desafios':
                ...
                $desafios = $this->model_pitacos->busca_desafios($id_rodada);
                $this->smarty->assign('desafios', $desafios);

                $rodada = $this->model_pitacos->busca_rodada($id_rodada);
                $this->smarty->assign('rodada', $rodada);

                $ja_apostou = $this->model_pitacos->ja_apostou($id_rodada, $this->usuario->Id_int);
                $this->smarty->assign('ja_apostou', $ja_apostou);
                
                if ($rodada->Liberacao == 'ok' && $rodada->Status_sel == 'aberta' && $ja_apostou) {
                    if ($this->usuario) {
                        if (!$this->model_pitacos->esta_desafiando($id_rodada, $this->usuario->Id_int)) {

                            $lista_possiveis = $this->model_pitacos->busca_apostadores_desafio($id_rodada, $this->usuario->Id_int);
                            foreach ($lista_possiveis as $possivel) {
                                if ($this->model_pitacos->ja_foi_convidado($id_rodada, $possivel->Id_int)) {
                ...
                $jogos_rodada = $this->model_pitacos->busca_jogos_rodada_completo($id_rodada);
            ...
            case 'convites':
                ...
                if ($rodada->Liberacao == 'ok' && $rodada->Status_sel == 'aberta') {
                    $lista_possiveis = $this->model_pitacos->busca_usuarios_convite($id_rodada, $this->usuario->Id_int);
                    $this->smarty->assign('lista_possiveis', $lista_possiveis);
                }
            ...
            case 'desafio':
                ...
                $desafio = $this->model_pitacos->busca_desafio($id_desafio);
            ...
            case 'responder_desafio':
                ...
                $rodada = $this->model_pitacos->busca_rodada($desafio->Rodadas_for);
                $this->smarty->assign('rodada', $rodada);
...
//          }
        ...
        switch ($segmento_post) {
            ...
            case 'envia_desafio':
                ...
                $desafio = $this->model_pitacos->insere_atualiza_desafio($_POST);

                if ($desafio) {

                    $desafio->Desafiante = $this->model_pitacos->busca_usuario($desafio->Desafiante_for);
                    $desafio->Desafiado = $this->model_pitacos->busca_usuario($desafio->Desafiado_for);

                    if ($this->model_pitacos->envia_email($_POST, 'desafio', TRUE, $desafio)) {
            ...
            case 'envia_convites':
                ...
                $rodada = $this->model_pitacos->busca_rodada($_POST['Rodadas_for']);
                $this->smarty->assign('rodada', $rodada);

                $usuario = $this->model_pitacos->busca_usuario($_POST['Users_for']);
                ...
                if ($_POST['convidados']) {
                    ...
                    foreach ($convites as $convite) {
                        $destinatario = $this->model_pitacos->busca_usuario($convite);
                        $dados->Destinatario = $destinatario;
                        $email['Destinatario_txf'] = $destinatario->Email_txf;
                        $enviou = $this->model_pitacos->envia_email($email, 'convite', TRUE, $dados);
                        if ($enviou) {
                            $cont = $cont + 1;
                        }
                    }
                ...
                $mensagem = "Enviado(s) $cont de $total convite(s)";
...
//busca_solicitacoes_pendentes
                ...
                $verifica_responder = $this->model_pitacos->verifica_responder($_POST);
//          ver('vedasdas');
                ...
                $verifica = $this->model_pitacos->verifica_inserir($_POST);
                if ($verifica->status == 'success') {


                    if ($this->model_pitacos->insere_atualiza_desafio($_POST)) {
