...
class Handler {
    ...
    public function openImapLocal($filePath) {
        ...
        $this->mailboxHandler = imap_open(
                $filePath, '', '', !$this->isNeutralProcessMode() ? CL_EXPUNGE : null
        );
    ...
    public function openImapRemote() {
        ...
        $this->mailboxHandler = imap_open(
                '{' . $this->mailboxHost . ':' . $this->mailboxPort . $opts . '}' . $this->mailboxName, $this->mailboxUsername, $this->mailboxPassword, !$this->isNeutralProcessMode() ? CL_EXPUNGE : null
        );
    ...
    /**
     * Process the messages by vedana.
     *
     * @return bool|Result
     */
    public function processaEmails() {
     ...
     * @return bool|Result
     */
    public function processMails($ultima_data = null) {
...
//            ver($ultima_data,1);
            if ($ultima_data) {
                $timestamp = strtotime($ultima_data);
                $data_string = date('d-M-Y', $timestamp);

                $mensagens = imap_search($this->mailboxHandler, 'SINCE "' . $data_string . '"', SE_UID);
            } else {
                $mensagens = imap_search($this->mailboxHandler, '', SE_UID);
            }


            $msgs = count($mensagens);
            echo "$msgs emails encontrados a partir de $data_string<br>";
//            array_splice($mensagens,50);

            foreach ($mensagens as $mensagem) {
                $email_number = imap_msgno($this->mailboxHandler, $mensagem);
                $header = @imap_fetchheader($this->mailboxHandler, $email_number);
                $body = @imap_body($this->mailboxHandler, $email_number);
             


//                $this->parse_parts($structure);

                $emeio = $this->processMailParsing($email_number, $header . '\r\n\r\n' . $body);
//                ver($emeio, 1);
                $cwsMbhResult->addMail($emeio);
            }
...
//            }
//        }
//        echo "$cont emails processados<br>";
    ...
    private function processMailParsing($token, $content) {
        ...
        if ($isFbl) {
            ...
            $header_parser=$this->parseHeader($bodySections['returned']);
                    $titulo=$header_parser['Subject'];
...
//                            ver();
            ...
            if (!self::isEmpty($arBodyMachine['perRecipient'])) {
                foreach ($arBodyMachine['perRecipient'] as $arRecipient) {
                    $cwsMbhRecipient = new Recipient();
                     $cwsMbhRecipient->setSubject($titulo);
...
//            $moveFolder = $this->getMailboxName().'.'.self::SUFFIX_BOUNCES_MOVE;
            $moveFolder = $this->getMailboxName() . '_processados';
//            ver($moveFolder);
            echo 'Process <strong>move ' . $cwsMbhMail->getType() . '</strong> in ' . $moveFolder . ' mailbox<br>';
            ...
            if ($this->isImapMailboxExists($moveFolder)) {
                ...
                return imap_mail_move($this->mailboxHandler, $cwsMbhMail->getToken(), $moveFolder, CP_MOVE);
            }
        } elseif ($this->isFileOpenMode()) {
//            ver('chegou');
    ...
    private function isImapMailboxExists($mailboxName, $create = true) {
        ...
        $handler = imap_open(
                '{' . $this->mailboxHost . ':' . $this->mailboxPort . $opts . '}', $this->mailboxUsername, $this->mailboxPassword, !$this->isNeutralProcessMode() ? CL_EXPUNGE : null
        );
        ...
        if (is_array($list)) {
            ...
            if ($mailboxFound === false && $create) {
                $mailboxFound = @imap_createmailbox(
                                $handler, imap_utf7_encode('{' . $this->mailboxHost . ':' . $this->mailboxPort . $opts . '}' . $mailboxName)
                );
            }
        }
     ...
     * @return array
     */
    public static function parseHeader($arHeader) {
