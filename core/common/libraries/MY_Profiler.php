    ...
    function run()
    {
		...
		if ($this->CI->config->item("firePHP") != "firebar")
		{
	    	...
	    	$i_currentDIV = rand(10,99).substr(md5($_SERVER["REQUEST_URI"]),0,6);
	   		$output .= "<div id='debugTrace".$i_currentDIV."X' style='position:absolute;top:0px;left:0px;width:10px;border:#FFFFEB solid 1px;filter:progid:DXImageTransform.Microsoft.gradient(startColorstr=#80000000,endColorstr=#10000000);background: rgba(135, 135, 135, 0.6);color:white;z-index:9999;'>";
	   		...
	   		$output .= "</div>";
	   		$output .= "<div id='debugTrace".$i_currentDIV."' class='debugTraceProfiler2' style='position:absolute;top:0px;left:10px;width:100%;border:#FFFFEB solid 1px;filter:progid:DXImageTransform.Microsoft.gradient(startColorstr=#80000000,endColorstr=#10000000);background: rgba(135, 135, 135, 0.6);color:red;z-index:9999;'>";
		  	$output .= "<table align='center' border='0' width='98%'>";
		   	$output .= "<tr>";
		   	$output .= "<td align='left' width='10%'><a href='Javascript:;' onclick='jQuery(\".codeigniter_profiler".$i_currentDIV."\").slideUp();jQuery(\"#codeigniter_profiler_URI".$i_currentDIV."\").slideDown();' style='text-decoration:none;font-size: 1em;color:#fff'> URI </a></td>";
...
//			$output .= $this->compile_queries();
	...
	{
           //   $type=$this->session->userdata['vars'];
			$output  = "\n\n";
			...
			$output .= "\n";
			$output .= '<legend style="color:#AA00FF;">&nbsp;&nbsp;'.$type.'&nbsp;&nbsp;</legend>';
			$output .= "\n";
...
//				foreach (CI::$APP->profiler_log[$type] as $key => $val)
    ...
    function compile_queries()
    {
		...
		foreach (CI::$APP as $key => $CI_object)
		{
